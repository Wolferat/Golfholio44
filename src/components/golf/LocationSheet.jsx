  ...
  return (
    <BottomSheet open={open} onClose={onClose} maxHeight="60dvh">
      <div className="p-5">
        ...
        <p className="text-sm text-muted-foreground mt-1">Set a home city or use your current location to sort what's nearby.</p>
        <button
          onClick={onUseGps}
          disabled={locating}
        ...
        >
          ...
          {locating ? 'Locating…' : hasCoords ? 'Using current location' : 'Use my current location'}
        </button>
        <div className="flex items-center gap-3 my-4 text-xs text-muted-foreground">
          <div className="h-px bg-border flex-1" /> or set a home city <div className="h-px bg-border flex-1" />
        </div>
        <div className="flex gap-2">
          <Input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="City or zip" className="h-12 rounded-2xl" />
          <Button onClick={() => onSave(draft || city)} className="h-12 rounded-2xl px-4">
